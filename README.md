...
- An import is limited to 1 MB and 1000 rules per file; a larger file is rejected rather than truncated. Every enabled rule is matched on each incoming call within a fixed deadline, so the cap keeps the whole rule set evaluable.
