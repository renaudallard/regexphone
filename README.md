...
- `java.util.regex.Pattern` has no built-in match timeout, and a regex with nested quantifiers like `(a+)+b` can backtrack catastrophically. RegexPhone runs every match on a watchdog thread with a 1 second deadline: a pattern that misses it is treated as *no match* and is skipped for the rest of the process lifetime, so the screening response always stays within Telecom's ~5 second budget. A runaway match cannot be cancelled mid-flight; at most four are kept on minimum-priority background threads, and while all four slots are stuck further matches are treated as no match.
